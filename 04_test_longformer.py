...
def detect(input_text,tokenizer,model,device='cuda:0',th=-3.08583984375):
    ...
    return label2decisions[decision], torch.nn.Softmax(dim=1)(outputs.logits)[0][0].item() #decision, machine-prob
...
DATASET = sys.argv[1] #obfuscated data filepath

CAST_DICT = {'text': 'string', 'generated': 'string', 'language': 'string', 'label': 'string', 'length': 'int16', 'source': 'string', 'domain': 'string', 'topic': 'string', 'split': 'string', 'multi_label': 'string', 'predictions': 'string', 'prediction_probs': np.float16}
multitude = pd.read_csv(DATASET, dtype = CAST_DICT)
multitude.fillna(value={"text": " ", "generated": " "}, inplace=True)

textfield = "text"
if 'generated' in multitude.columns:
  #textfield = "generated"
  multitude.rename(columns={"text": "text_original", "generated": "text"}, inplace=True)
  multitude['label'] = ["human" if "human" in x else "machine" for x in multitude.multi_label]

predictions = []
probs = []
for text in tqdm(multitude[textfield]):
  temp = preprocess(text)
  result = detect(temp,tokenizer,model,device)
  predictions.append(result[0])
  probs.append(result[1])

multitude['predictions'] = predictions
multitude['prediction_probs'] = probs

multitude['predictions'] = ["human" if str(x) == "0" else "machine" for x in multitude['predictions']]

multitude.to_csv(DATASET.replace(".csv","_detection-longformer.csv"), index=False)
print(classification_report(multitude['label'], multitude['predictions'], digits=4))